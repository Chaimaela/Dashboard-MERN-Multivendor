...
const Orders = () => {
    ...
    return (
        <div className='px-2 lg:px-7 pt-5'>
            <div className='w-full p-4 bg-[#1d1c22] rounded-md'>
                <div className='flex justify-between items-center'>
                    <select onChange={(e) => setParPage(parseInt(e.target.value))} className='px-4 py-2 focus:border-slate-500 outline-none bg-[#1d1c22] border border-slate-700 rounded-md text-[#fff]'>
                        <option value="5">5</option>
                        <option value="10">10</option>
                        <option value="20">20</option> 
                    </select>
                    <input onChange={e => setSearchValue(e.target.value)} value={searchValue} className='px-4 py-2 focus:border-slate-500 outline-none bg-[#1d1c22] border border-slate-700 rounded-md text-[#fff]' type="text" placeholder='search' /> 
                </div>


        <div className='relative mt-5 overflow-x-auto'>
            <div className='w-full text-sm text-left [#d0d2d6]'>
                <div className='text-sm text-[#fff] uppercase border-b border-slate-700'>
        ...
        {
            myOrders.map((o,i) =>  <div className='text-[#fff] '>
